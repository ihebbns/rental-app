...
export default function ContractDetailsPage({ params }) {
  ...
  const handleReturnSubmit = async () => {
    ...
    try {
      ...
      router.push(`/cars/${carId}/contracts`); // ✅ Redirection version française
    } catch (error) {
  ...
  return (
    <div className="min-h-screen bg-gray-100 p-8">
      <h1 className="text-3xl font-bold text-gray-800 mb-6 text-center">📄 Détails du Contrat</h1>

      <div className="bg-white shadow-lg rounded-lg p-6 max-w-2xl mx-auto">
        <h2 className="text-2xl font-semibold text-gray-800">{contract.customerName}</h2>
        <p className="text-gray-600 mt-2">
          📆 Période de location : <strong>{new Date(contract.rentalStartDate).toLocaleDateString()}</strong> - <strong>{new Date(contract.rentalEndDate).toLocaleDateString()}</strong>
        </p>
...
{/* 🔹 Photo Gallery */}
{contract.photos.length > 1 && ( // Ensure there's more than just the contract photo
  <div className="mt-6">
    <h3 className="text-gray-700 font-semibold">📷 Photos du véhicule :</h3>
    <div className="grid grid-cols-2 gap-4 mt-2">
      {contract.photos.slice(1).map((photo, index) => ( // Start from the second photo
        <img
          key={index + 1} // Keep keys unique
          src={photo}
          alt={`Photo ${index + 1}`} // This makes the first vehicle photo start from "Photo 1"
          className="w-full h-32 object-cover rounded-md shadow cursor-pointer transition-transform transform hover:scale-105"
