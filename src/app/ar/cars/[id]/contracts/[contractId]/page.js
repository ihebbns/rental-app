...
export default function ContractDetailsPage({ params }) {
  ...
  const handleReturnSubmit = async () => {
    if (!returnMileage || isNaN(returnMileage) || returnMileage <= 0) {
      alert("يرجى إدخال عدد كيلومترات صالح.");
      return;
    }
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error("فشل في إرسال إرجاع السيارة.");
      }
  
      alert("تم تسجيل إرجاع السيارة بنجاح. العقد مؤرشف.");
      router.push(`/ar/cars/${carId}/contracts`); // ✅ Redirection version française
    } catch (error) {
      console.error("خطأ أثناء إرجاع السيارة:", error);
      alert("فشل في إرسال الإرجاع.");
    }
  };
